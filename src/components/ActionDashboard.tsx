...
import { Volume2, ChevronRight, Search, Mic, Zap, Shield, Heart, Loader2, Sparkles, X, Save, ArrowRight, Database, Plane, Accessibility } from 'lucide-react';
...
export default function ActionDashboard() {
  ...
  // Modos visuales: normal (softening), roleplay (vuelo), shadow (pronunciación)
  const [learningMode, setLearningMode] = useState<'normal' | 'roleplay' | 'shadow'>('normal');
  ...
  useEffect(() => {
    // Si cambia el trustMode, limpiamos el resultado para evitar confusiones
    setSoftenedResult(null);
  }, [trustMode, learningMode]);
  ...
  useEffect(() => {
    const fetchUnits = async () => {
      ...
      try {
        const mappedProfession = PROFESSION_MAP[userProfile.occupation] || 'general';
  ...
  // En lugar de requerir que se pulse el micro primero, podemos activar el always-on al iniciar
  useEffect(() => {
    // Si queremos always on por defecto, podemos descomentar esto
    // handleAnalyMic();
  }, []);
  ...
  const startListeningLoop = () => {
    ...
    recognitionRef.current = startSpeechAPI(
      (text, isFinal) => {
        ...
        const wakeWords = ['analy', 'analí', 'anali', 'analee', 'annaly', 'ana lee', 'ana li'];
        let earliestIndex = -1;
        let bestTrigger = '';
        ...
        if (earliestIndex !== -1) {
          let phrase = lowerText.substring(earliestIndex + bestTrigger.length).trim();
          
          if (isFinal) {
            if (phrase.length > 2) {
              setInterimTranscript(`Procesando Fichas: "${phrase}"...`);
              isAnalyActiveRef.current = false; 
              recognitionRef.current?.stop(); 
              
              // Doble Capa Check
              let forceClientMode = false;
              if (phrase.includes('dile') || phrase.includes('dile a') || phrase.includes('dile al')) {
                 forceClientMode = true;
              }
              
              processIntention(phrase, learningMode, forceClientMode);
            } else {
              setInterimTranscript('Anali: ¿Sí?');
            }
          } else {
             setInterimTranscript(`Anali escuchando... ${phrase}`);
          }
        } else {
  ...
  const processIntention = async (phrase: string, forceMode?: 'normal' | 'roleplay' | 'shadow', forceClientMode?: boolean) => {
    setIsProcessing(true);
    setInterimTranscript('Conectando a Anali...'); 
    
    const lowerPhrase = phrase.trim().toLowerCase();
    const cleanPhrase = lowerPhrase.replace(/^(analy|analí|anali|analee|annaly|ana lee|ana li)[\s,:-]*/, '').replace(/^dile al?[\s,:-]*/, '').replace(/^dile[\s,:-]*/, '');
    
    // Determine the exact logic mode
    ...
    try {
      ...
      if (result && !result.errorMsg) {
        ...
        if (result.is_navigation) {
           speak(result.assistant_reply || 'Vale, te llevo ahí.', true);
           if (result.nav_scenario) {
              localStorage.setItem('analy_nav_scenario', result.nav_scenario);
           }
        ...
        speak(result.corrected_en || result.softened, true); 
        setInterimTranscript(''); 
      } else {
  ...
  const speak = async (text: string, resumeListening: boolean = false) => {
    const utterance = await globalSpeak(text, 'en-US');
    ...
    <div className="p-4 space-y-6 min-h-[85vh]">
      ...
      {learningMode === 'normal' && (
        <div className="space-y-4">
          ...
          {/* Dynamic Result Card based on Mode */}
          <AnimatePresence>
            {softenedResult && (
              <motion.div 
                ...
                {cardView === 'client' ? (
                  ...
                  <div className="space-y-5">
                    ...
                    <div className="px-2 border-t border-slate-100 pt-4">
                       ...
                       <p className="text-sm text-slate-600 leading-relaxed font-medium bg-teal-50/50 p-3 rounded-xl border border-teal-100/50">
                    ...
                    <div className="flex items-center gap-3 pt-2">
                      <button onClick={() => speak(softenedResult.corrected_en || softenedResult.softened, false)} className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-teal-500 text-white rounded-xl text-xs font-bold shadow-md shadow-teal-500/20 active:scale-95 transition-all">
      ...
      {learningMode === 'shadow' && (
       ...
       {/* Ocultar Categorías si no estamos en modo normal, para no distraer */}
       {learningMode === 'normal' && (
      ...
      {/* Lista Táctica solo en modo normal */}
      {learningMode === 'normal' && (
        <div className="space-y-4">
                  ...
                  <div className="py-20 text-center space-y-2">
                    {learningUnits.length === 0 ? (
                      <div className="opacity-70 bg-slate-50 p-6 rounded-3xl border border-slate-100">
                         ...
                         <h3 className="text-lg font-black text-slate-700 mb-2">Contenido de {userProfile?.occupation} en desarrollo.</h3>
...
interface ScriptCardProps {
  script: LearningUnit;
  onSpeak: (text: string, resumeListening?: boolean) => void;
}
              ...
              {script.learning_tips && script.learning_tips.length > 0 && (
                <div className="bg-amber-50 rounded-xl p-3">
                  ...
                  <ul className="text-xs text-slate-600 space-y-1 font-medium pl-2">
                    {script.learning_tips.map((tip, idx) => <li key={idx}>- {tip}</li>)}
                  </ul>
                </div>
              )}
            </div>
